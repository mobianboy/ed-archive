...
use \Eardish\EmailService\ServiceKernel;
use \Eardish\EmailService\EmailService;
...
const CONFIG_SERVICE_NAME = 'email';

// Name used for other output
const ACTUAL_SERVICE_NAME = 'Email Service';
...
$kernel->setService(new EmailService(new Connection(), $kernel, $config));
...
$back->on('connection', function( $conn) use ($kernel, $log, $serviceLog){
    ...
    $conn->on('data', function($data) use ($conn, $kernel, $log, $serviceLog) {
        ...
        try {
            ...
            $kernel->processException($e, $requestId);
        }
    });
});
...
$cli->bold()->green(ACTUAL_SERVICE_NAME . ' is up at address '.$host.' on port ' . $frontPort. ' :: Monitoring on port ' . $config->get(CONFIG_SERVICE_NAME.'.monitor.port'));
