...
# this script is used for removing all the service apps from all the servers  
...
echo Starting script to remove the services
date
...
##
echo deleting all folders under /eda/app/
for server in ${servers[@]}
do
ssh $server "sudo rm -rf /eda/app/*"
done
