...
cd /home/$admin/$environment/code

# git clone git@github.com:eardish/artools-client.git -b dev
git clone git@github.com:eardish/conductor-gateway.git -b dev
git clone git@github.com:eardish/email-service.git -b dev
git clone git@github.com:eardish/auth-service.git -b dev
git clone git@github.com:eardish/conductor-bridge.git -b dev
git clone git@github.com:eardish/user-service.git -b dev
git clone git@github.com:eardish/profile-service.git -b dev
git clone git@github.com:eardish/recommendation-service.git -b dev
git clone git@github.com:eardish/analytics-service.git -b dev
git clone git@github.com:eardish/music-service.git -b dev
git clone git@github.com:eardish/image-processing-service.git -b dev
git clone git@github.com:eardish/song-ingestion-service.git -b dev
# git clone git@github.com:eardish/conductor-dataobjects.git 
# git clone git@github.com:eardish/db-cron.git
git clone git@github.com:eardish/notation.git -b dev
cd /home/$admin/$environment/code/$environment
...
do
ssh "$server" sudo bash /home/$admin/devops/scripts/services/$environment/stop_PHP_services.sh
done
...
for server in ${servers[@]}
do
rsync -r -e ssh /home/$admin/$environment/code/$environment/configs/secret/ $admin@$server:$configs
done
...
do
rsync -r -e ssh /home/$admin/$environment/code/conductor-gateway $admin@$gateway_app:$edservices
done

echo deploying bridge
for bridge_app in ${bridge[@]}
do
rsync -r -e ssh /home/$admin/$environment/code/conductor-bridge $admin@$bridge_app:$edservices
done

echo deploying email
for email_app in ${email[@]}
do
rsync -r -e ssh /home/$admin/$environment/code/email-service $admin@$email_app:$edservices
done

echo deploying auth
for auth_app in ${auth[@]}
do
rsync -r -e ssh /home/$admin/$environment/code/auth-service $admin@$auth_app:$edservices
done

echo deploying rec
for rec_app in ${rec[@]}
do
rsync -r -e ssh /home/$admin/$environment/code/recommendation-service $admin@$rec_app:$edservices
done

echo deploying song
for song_app in ${song[@]}
do
rsync -r -e ssh /home/$admin/$environment/code/song-ingestion-service $admin@$song_app:$edservices
done

echo deploying profile
for profile_app in ${profile[@]}
do
rsync -r -e ssh /home/$admin/$environment/code/profile-service $admin@$profile_app:$edservices
done

echo deploying music
for music_app in ${music[@]}
do
rsync -r -e ssh /home/$admin/$environment/code/music-service $admin@$music_app:$edservices
done

echo deploying image
for image_app in ${image[@]}
do
rsync -r -e ssh /home/$admin/$environment/code/image-processing-service $admin@$image_app:$edservices
done

echo deploying user
for user_app in ${user[@]}
do
rsync -r -e ssh /home/$admin/$environment/code/user-service $admin@$user_app:$edservices
done

echo deploying dbsvc
for dbsvc_app in ${dbsvc[@]}
do
rsync -r -e ssh /home/$admin/$environment/code/notation $admin@$dbsvc_app:$edservices
done

echo deploying analytics
for analytics_app in ${analytics[@]}
do
rsync -r -e ssh /home/$admin/$environment/code/analytics-service $admin@$analytics_app:$edservices
done
...
for gateway_app in ${gateway[@]}
do
ssh $admin@$gateway_app /home/$admin/devops/scripts/$environment/deployment_tasks/task_composer/composer_gateway.sh
done

for bridge_app in ${bridge[@]}
do
ssh $admin@$bridge_app /home/$admin/devops/scripts/$environment/deployment_tasks/task_composer/composer_bridge.sh
done

for email_app in ${email[@]}
do
ssh $admin@$email_app /home/$admin/devops/scripts/$environment/deployment_tasks/task_composer/composer_email.sh
done

for auth_app in ${auth[@]}
do
ssh $admin@$auth_app /home/$admin/devops/scripts/$environment/deployment_tasks/task_composer/composer_auth.sh
done

for rec_app in ${rec[@]}
do
ssh $admin@$rec_app /home/$admin/devops/scripts/$environment/deployment_tasks/task_composer/composer_rec.sh
done

for song_app in ${song[@]}
do
ssh $admin@$song_app /home/$admin/devops/scripts/$environment/deployment_tasks/task_composer/composer_song.sh
done

for profile_app in ${profile[@]}
do
ssh $admin@$profile_app /home/$admin/devops/scripts/$environment/deployment_tasks/task_composer/composer_profile.sh
done

for music_app in ${music[@]}
do
ssh $admin@$music_app /home/$admin/devops/scripts/$environment/deployment_tasks/task_composer/composer_music.sh
done

for image_app in ${image[@]}
do
ssh $admin@$image_app /home/$admin/devops/scripts/$environment/deployment_tasks/task_composer/composer_image.sh
done

for user_app in ${user[@]}
do
ssh $admin@$user_app /home/$admin/devops/scripts/$environment/deployment_tasks/task_composer/composer_user.sh
done

for dbsvc_app in ${dbsvc[@]}
do
ssh $admin@$dbsvc_app /home/$admin/devops/scripts/$environment/deployment_tasks/task_composer/composer_dbsvc.sh
done

for analytics_app in ${analytics[@]}
do
ssh $admin@$analytics_app /home/$admin/devops/scripts/$environment/deployment_tasks/task_composer/composer_analytics.sh
done
