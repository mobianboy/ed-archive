...
#This script will check to see if Image Processing php process is running and if not then it will start the service
...
if "ps -eo pid,command | awk '!/awk/ && /image/ {print $1}'" &>/dev/null; then
    printf "PHP is running\n"
else
   echo starting php service
   bash  /home/$admin/devops/scripts/services/$environment/start_image.sh
fi
