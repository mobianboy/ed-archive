...
#This script will check to see if email php process is running and if not then it will start the service
...
if "ps -eo pid,command | awk '!/awk/ && /analytics/ {print $1}'" &>/dev/null; then
   ...
   bash  /home/$admin/devops/scripts/services/$environment/start_email.sh
fi
