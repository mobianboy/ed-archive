...
use \Eardish\AnalyticsService\ServiceKernel;
use \Eardish\AnalyticsService\AnalyticsService;
...
const CONFIG_SERVICE_NAME = 'analytics';

// Name used for other output
const ACTUAL_SERVICE_NAME = 'Analytics Service';
...
$kernel->setService(new AnalyticsService(new Connection(), $kernel, $config));
...
$back->on('connection', function( $conn) use ($kernel, $log, $serviceLog){
    ...
    $conn->on('data', function($data) use ($conn, $kernel, $log, $serviceLog) {
        ...
        try {
            ...
            $kernel->processException($e, $serviceId);
        }
    });
});
