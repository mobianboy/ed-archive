...
use \Eardish\UserService\ServiceKernel;
use \Eardish\UserService\UserService;
use \Eardish\UserService\Core\Connection;
...
const CONFIG_SERVICE_NAME = 'user';

// Name used for other output
const ACTUAL_SERVICE_NAME = 'User Service';
...
// set up the handler that converts notices to exceptions
...
// Set up the ServiceKernel while passing in an instance of the actual Service and a logger

$kernel = new ServiceKernel($serviceLog);
$kernel->setService(new UserService(new Connection(), $kernel, $config));
...
$back->on('connection', function( $conn) use ($kernel, $log, $serviceLog, &$serviceId){
    ...
    $conn->on('data', function($data) use ($conn, $kernel, $log, $serviceLog, &$counter) {
        ...
        try {
            ...
            $kernel->processException($exception, $requestId);
        }
    });
});
