...
class ServiceKernel
{
    ...
    {
        $this->log = $log;
        $this->reflector = new \ReflectionClass("Eardish\\ProfileService\\ProfileService");
    }


    public function setService(ProfileService $service)
    {
    ...
    public function processException(\Exception $ex, $serviceId)
    {
        ...
        $exceptionArray["exception"]["requestId"] = $this->requests[$serviceId]['requestId'];
        $this->service->setPort('bridge.back.port', true);
        $this->service->setAddr($this->requests[$serviceId]['fqdn']);
